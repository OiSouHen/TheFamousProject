...
RegisterCommand("cds",function(source)
	...
	if Passport and GlobalState["Commands"] then
		if vRP.HasGroup(Passport,"Admin",2) then
			...
			vKEYBOARD.Copy(source,"Cordenadas:",SplitUnique(Coords["x"])..","..SplitUnique(Coords["y"])..","..SplitUnique(Coords["z"])..","..SplitUnique(Heading))
		else
			TriggerClientEvent("Notify",source,"amarelo","Você não tem permissões para isso.","Atenção",5000)
		end
	end
...
RegisterCommand("announce",function(source)
	...
	if Passport and GlobalState["Commands"] then
		if vRP.HasGroup(Passport,"Admin") then
			...
			if Keyboard then
				TriggerClientEvent("Notify",-1,Keyboard[1],Keyboard[4],Keyboard[3],Keyboard[2] * 1000)
