...
function vRP.InsidePropertys(Passport, Coords)
	...
	if HensaTable then
		HensaTable["Pos"] = { x = SplitUnique(Coords["x"]), y = SplitUnique(Coords["y"]), z = SplitUnique(Coords["z"]) }
	end
end
-----------------------------------------------------------------------------------------------------------------------------------------
-- DISCONNECT
			...
			if GetResourceMetadata("vrp", "creator") == "yes" then
				if vRP.UserData(Passport, "Creator") == 1 then
					if Global[Passport] then
						TriggerClientEvent("spawn:Finish", source, vec3(HensaTable["Pos"]["x"], HensaTable["Pos"]["y"], HensaTable["Pos"]["z"]), false)
					else
						TriggerClientEvent("spawn:Finish", source, vec3(HensaTable["Pos"]["x"], HensaTable["Pos"]["y"], HensaTable["Pos"]["z"]), false)
					end
				else
