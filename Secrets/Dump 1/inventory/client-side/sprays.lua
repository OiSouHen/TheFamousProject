...
CreateThread(function()
	for _,v in pairs(Sprays) do
		local Object = CreateDui(v["Texture"],512,512)
		local Texture = CreateRuntimeTxd("sprayTxd_"..v["Permission"])
		local DuiHandler = GetDuiHandle(Object)

		CreateRuntimeTextureFromDuiHandle(Texture,"sprayTex_"..v["Permission"],DuiHandler)
	end
end)
-----------------------------------------------------------------------------------------------------------------------------------------
-- SPRAYCONTROLLING
-----------------------------------------------------------------------------------------------------------------------------------------
function Creative.SprayControlling(Permission)
	local Model = "sprays"
	...
	if LoadModel(Model) then
		...
		while Progress do
			...
			if RayCoords and RayNormal then
				...
				AddReplaceTexture("sprays","np_sprays_st_dif","sprayTxd_"..Permission,"sprayTex_"..Permission)
